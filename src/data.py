...
def highest_paying_engineer(df_sorted):
    ...
    print(f"The job that pays the most is {top_engineers_job}")
...
def compare_job_region(job, region):
...
def find_job_with_biggest_diff(level):
    ...
    # Print the result
    print(
        "The job with the largest overall salary difference is: {}".format(max_diff_job)
    )
